...
export function CubeScene({ cubeState, animation, onAnimationComplete }: Props) {
  return (
    <section className="panel relative h-[clamp(520px,calc(100vh-260px),760px)] overflow-hidden rounded-lg bg-[#edf3fb]">
      ...
      <div className="pointer-events-none absolute left-4 top-4 rounded-md border border-slate-200 bg-white/90 px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur">
        Interactive 3D cube
      </div>
      <div className="pointer-events-none absolute bottom-4 right-4 rounded-md border border-slate-200 bg-white/85 px-3 py-2 text-xs font-medium text-slate-600 shadow-sm backdrop-blur">
        Drag to orbit
      </div>
    </section>
  );
